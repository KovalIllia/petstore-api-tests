...
def test_find_pet_by_id(pet_api, pet_payload):
    ...
    for key in ["id", "category", "name", "photoUrls", "tags", "status"]:
        assert key in created_pet, f"Missing key in response: {key}"
    pet_id = created_pet["id"]
    assert pet_id is not None, f"Pet ID should not be None"

    PetWaiter.wait_for_pet(pet_api, pet_id, expected_status=200)
    found_animal_response = pet_api.find_pet_by_id(pet_id)
    assert found_animal_response.status_code == 200, f"unsuccessful attempt to find a pet by id: {pet_id}"

    found_animal_data = found_animal_response.json()
    assert found_animal_data["id"] == pet_id, f"Expected pet_id: {pet_id}, but got: {found_animal_data['id']}"
    assert len(found_animal_data) > 0, f"Empty object of pet"
...
def test_update_pet_with_form(pet_api, pet_payload):
    ...
    pet_response = PetWaiter.wait_for_pet(pet_api, pet_id, expected_status=200)
...
def test_delete_pet(pet_api, pet_payload):
    ...
    found_animal_response = pet_api.find_pet_by_id(pet_id)
    assert found_animal_response.status_code == 404, f"Pet with id: {pet_id} was not deleted"
...
def test_upload_pet_image(pet_payload, files_client, pet_api):
    ...
    assert len(pet_data_after_upload["photoUrls"])>0,"photoUrls is empty after image upload"
